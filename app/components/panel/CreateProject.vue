<template>
  <section>
    ...
    <div v-if="projectFields.enable_budget">
      <budget-builder
        v-if="projectFields.enabled_detailed_budget"
        ref="budget"
        :budget-limit="projectFields.budget_limit"
        :editable="true"
      ></budget-builder>
      <budget-simple
        v-else
    ...
    <div class="message is-success" v-show="response.replied && response.ok">
      <div class="message-body">
        ...
        <b>¡Genial!</b> El proyecto ha sido guardado correctamente!
      </div>
    </div>
...
export default {
  props: ["formUrl", "categories", "districts", "projectFields", "user"],
  components: {
    DefaultFields,
    ProjectFields,
    BudgetSimple,
  ...
  methods: {
    ...
    makePayload: function() {
      let payload = {
        ...
        organization_address: this.isOptional(
          this.project.organization_address
        ),
        ...
        .then(responses => {
          if (!this.checkEverythingIsTrue(responses)) {
            ...
            this.$buefy.snackbar.open({
              message:
                "Faltan datos o algunos campos son incorrectos. Verifíquelos.",
              type: "is-danger",
              actionText: "Cerrar"
            });
            return;
